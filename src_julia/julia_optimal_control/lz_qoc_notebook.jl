...
"""
function d_expm_dx(A::AbstractMatrix, E::AbstractMatrix; method::Symbol = :eig)
	if size(A) != size(E)
...
function run_landau_zener_optimization()
	...
	for attempt in 1:Nattempts
		for idx_T in length(Tfs):-1:1  # longest time first
			Tf = Tfs[idx_T] * T0
			t = collect(range(0.0, Tf; length = Nts + 1))
			dt_local = t[2] - t[1]

			@printf("Run %d of %d, attempt %d of %d\n", idx_T, length(Tfs), attempt, Nattempts)
			...
			if c_const != 0.0
				...
				inner = LBFGS()
				opts = Optim.Options(g_tol = grad_tol, show_trace = false, iterations = 200, time_limit = 30.0)
				res = optimize(cost_fun, grad_fun!, lower, upper, controls_0, Fminbox(inner), opts)
			else
				opts = Optim.Options(g_tol = grad_tol, show_trace = false, iterations = 200, time_limit = 30.0)
	...
	if length(Tfs) == 1
		...
		plot!(p, edges[1:(end-1)], initial_controls;
			linetype = :steppost, linestyle = :dash, linewidth = 1.5,
			color = :gray, label = "Initial")

		plot!(p, edges[1:(end-1)], controls_opt;
