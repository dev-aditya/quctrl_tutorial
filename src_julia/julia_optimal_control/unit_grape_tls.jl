...
const ID2 = I(2)
const OMEGA = 1.0  # energy unit
const T0 = π / (2 * OMEGA)  # unit of time
const T = pi / OMEGA        # total evolution time (unused but kept for reference)
...
Tfs = [1]                               # run like this to see the actual fields
fide_opt = zeros(length(Tfs))
dt = Tfs / Nts                           # ensure Float64 like Python's 2/Nts
...
ang = fac*pi  # angle of target Pauli rotation
U_target = exp(-im * ang * SIGMA_Z / 2)  # target unitary operator
# -----------------------
...
function single_qubit_gate(α_t::Real)::Matrix{ComplexF64}
    return OMEGA * (cos(α_t) * SIGMA_X + sin(α_t) * SIGMA_Y) / 2
end
...
function fidelity(Uf::AbstractMatrix, U_target::AbstractMatrix)::Float64
    if size(Uf) != size(U_target)
        error("Input unitaries must have the same dimensions.")
    end
    d = size(Uf, 1)
    return abs(tr(adjoint(U_target) * Uf))^2 / d^2
end
...
function cost_from_propagators(
    ...
    Uf = propagators[end]
    return 1.0 - fidelity(Uf, U_target)
end
...
function control_gradient_from_propagators(
    ...
    for j = 1:n_steps
        hj = single_qubit_gate(controls[j])
        dH_da = OMEGA * (-sin(controls[j]) * SIGMA_X + cos(controls[j]) * SIGMA_Y) / 2
        duj_dcj = d_expm_dx(-im * hj * dt, -im * dH_da * dt; method = :block)
...
function control_gradient_from_propagators(
    ...
    U_total = propagators[end]
    d = size(U_total, 1)
    z = tr(adjoint(U_target) * U_total) / d
    ...
    for j = 1:n_steps
        ...
        dU_dcj = U_backward * duj_dcj * U_forward
        dz_dcj = tr(adjoint(U_target) * dU_dcj) / d
...
function control_cost(
    ...
    propagators = forward_propagators(controls, dt)
    return cost_from_propagators(propagators, U_target)
end
...
function control_gradient!(
    ...
    propagators = forward_propagators(controls, dt)
    grad_out .= control_gradient_from_propagators(controls, propagators, U_target, dt)
    return nothing
end
...
c_const = Float64(π)  # if set to 0.0, there is no constraint
...
control_gradient!(_warm_grad, _warm_x, U_target, 0.1)
_ = control_cost(_warm_x, U_target, 0.1)
...
for attempt = 1:Nattempts
    for idx_T = length(Tfs):-1:1  # longest time first
        ...
        cost_fun = x -> control_cost(x, U_target, dt_local)
        grad_fun! = (g, x) -> control_gradient!(g, x, U_target, dt_local)

        # Initialize control field
        if idx_T == length(Tfs)
            if guess == "random"
                controls_0 =
                    rand(LinRange(nextfloat(-c_const), prevfloat(c_const), 1000), Nts)
            elseif guess == "zero"
                controls_0 = Vector(LinRange(nextfloat(-c_const), prevfloat(c_const), Nts))
            else
...
if length(Tfs) == 1
    cost_x0 = control_cost(initial_controls, U_target, 0.1)
    p = plot(
        size = (500, 300),
        title = @sprintf("cost0=%.5f, costF=%.2E", cost_x0, final_fide),
        titlefontsize = 8,
    )
