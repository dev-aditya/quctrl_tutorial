...
ReplayBuffer(capacity::Int = 10_000) =
    ReplayBuffer(Tuple{Vector{Float64},Int,Float64,Vector{Float64},Bool}[], capacity)
...
function DQNAgent(
    ...
    return DQNAgent(net, Float32(lr), γ, ε, ε_decay, ε_min, batch, ReplayBuffer(buffer_cap))
end
...
function learn!(agent::DQNAgent, rng::AbstractRNG)
    ...
    rewards = Flux.f32.([t[3] for t in batch])
    ...
    function loss_fn()
        ...
        targets = Float32.([
            rewards[i] + (dones[i] ? 0.0 : agent.γ * next_q[1, i]) for
            i in eachindex(rewards)
        ])
        return Flux.Losses.mse(chosen, targets)
    end

    ps = Flux.params(agent.net)
    grads = Flux.gradient(() -> loss_fn(), ps)
    for p in ps
        g = grads[p]
        p .-= agent.lr .* g
    end
...
function run_dqn(; episodes = 300, rng = Random.default_rng())
    ...
    for ep = 1:episodes
        ...
        while !done
            ...
            push!(
                agent.buffer,
                (Float32.(state), action, Float32(reward), Float32.(next_state), done),
            )
            learn!(agent, rng)
            state = next_state
            total += reward
    ...
    env.state = [2.0f0, 0.0f0]
    env.step = 0
    for t = 0:20
        pos, vel = env.state
        action = choose_action(agent, Float64.(env.state), rng)
